...
never reaches state. This is a different arrangement, not a drop-in
replacement: RDS creates its *own* secret and never reads a pre-existing one,
so anything already authenticating with that secret has to be repointed at
`aws_db_instance.this.master_user_secret`. Only reach for it when no specific
secret has to be honoured.
